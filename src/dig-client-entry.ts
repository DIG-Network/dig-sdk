...
// integrity model as the main entry. Consumers that want JUST the read path (e.g. `dig-embed.js`,
// a worker) import from here instead of the whole SDK.
//
...
// browser: the `web` build). Integrity anchors per the load path: byte-level SRI (fail-closed) on the
// Node path and explicit configureWasm({ wasmBytes | wasmUrl }) path; pinned-package trust on the
// default browser (bundler) path — same model as `src/loader.ts`. This entry is a re-export of that
// loader/client, not a reimplementation — its public surface is stable.
